...
import uia.core.event.OnMouseEnter;
import uia.core.event.OnMouseExit;
...
public class UICalendar extends WrapperView {
    ...
    public UICalendar(View view) {
        ...
        listUI.getViewRight().addEvent((OnClick) (v, pointers) -> setDate(sDay, sMonth + 1, sYear));
        listUI.getViewLeft().addEvent((OnClick) (v, pointers) -> setDate(sDay, sMonth - 1, sYear));
        ...
        for (int i = 0; i < 31; i++) {
            ...
            cell.addEvent((OnClick) (v, pointers) -> {
            ...
            cell.addEvent((OnMouseEnter) (v, pointers) -> {
                Paint paint = v.getPaint();
                if (!paint.equals(paintCell[2])) paint.set(paintCell[1]);
            });
            cell.addEvent((OnMouseExit) (v, pointers) -> {
                Paint paint = v.getPaint();
    ...
    public interface OnSelection extends Event<View, Integer> {
    }
    ...
    public interface OnChange extends Event<View, Integer[]> {
    }
    ...
    public void setDate(int d, int m, int y) {
        ...
        offset = Utils.getDay(calendar.get(Calendar.DAY_OF_WEEK));
