...
/**
 * Figure is used to build standard shapes
 */
...
public final class Figure {
    ...
    public static Geom triangle(Geom geom, int vertices, float a, float b, float c) {
        geom = checkValidity(geom, 3);
        geom.clear();

        vertices = vertices / 3;

        float div = geom.width() / geom.height();
        float f1 = 1, f2 = 1;
        ...
        for (int i = 0; i <= vertices; i++) {
            ...
            geom.addVertex(1 - bx + bx * cos(an), -by * sin(an));
        }
