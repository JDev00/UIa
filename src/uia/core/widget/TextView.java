...
import uia.structure.list.BChar;
 ...
 * <br>
 * <b>Text properties</b> such as: longest line, justification and so on are calculated at render time.
 * <br>
...
public class TextView extends View {
    ...
    private final BChar dBuffer;
    private final BInt dLines;
    protected final BChar buffer;
    protected final BInt lines;
    ...
    public void setTextBound(float textBound) {
        this.textBound = max(0f, textBound);
    }
    ...
    public final int getLineHeight() {
        return (int) (lineFactor * 1.33f * font.getSize());
    }
    ...
    private void updateCursorPosParams(int index) {
        ...
        float lineLength = font.getWidth(buffer.toArray(), j, index - j);

        if (isAlignX(AlignX.LEFT)) {
            cOffX = lineLength;
        } else if (isAlignX(AlignX.CENTER)) {
            cOffX = lineLength - font.getWidth(buffer.toArray(), j, br - j) / 2;
        }
    }
    ...
    private void drawSelected(Render render, Paint paint, int px, int py, int ax) {
        ...
        float offset = font.getWidth(buffer.toArray(), j, minIndex - j);
        ...
        while (i < lines.size() && br < maxIndex) {
            ...
            float lineWidth = font.getWidth(buffer.toArray(), j, br - j);
            int width = (int) font.getWidth(buffer.toArray(), minIndex, Math.min(maxIndex, br) - minIndex);
     ...
     * @param buffer the buffer to draw on screen
    ...
    private void drawText(Render render,
        ...
        float bot = py() + dy() / 2f;
        int lineHeight = getLineHeight();
        int off = (int) (font.getSize() / 3f);

        int i = 0;
        int j;
        int br = 0;

        char[] data = buffer.toArray();

        while (i < lines.size()) {
            j = br;
            br = lines.get(i);

            int lineLength = (int) font.getWidth(data, j, br - j);
    ...
    private int getIndex(int px, int py,
        ...
        while (i < lines.size()) {
            ...
            if (Utils.AABB(mx, my, 1, 1, dx() / 2, py + height - fSize / 2f, dx(), fSize)) {
                float d;
                float length = 0;
                float xLength = 0.5f * xOff * font.getWidth(buffer.toArray(), j, br - j);

                // There is a problem here. Error index calculation
                while (j < br && !Utils.AABB(mx, my, 1, 1, px + xLength + length
                        + (d = font.getWidth(buffer.get(j))) / 2, py + height - fSize / 2f, d, fSize)) {
                    length += d;
                    j++;
                }
    ...
    protected void postDraw(Render render) {
        ...
        if (requestJustification
            ...
            if (singleLine) {
                ...
                justify(buffer, lines, font, autoJustification ? textBound * dx() : Float.MAX_VALUE);
            }
        }
     ...
     * Space required: O(1)
     *
     * @param buffer a not null {@link BChar}
     * @param lines  a not null {@link BInt} used to store break line indices
     * @param font   a not null {@link Font} used to compute the width of each character
     * @param width  a value {@code > 0} used to set the maximum line's width in pixel
     */

    public static void justify(BChar buffer, BInt lines, Font font, float width) {
        if (width >= 0
                && buffer != null
                && lines != null
                && font != null) {
            ...
            buffer.add(lines.toArray(), 0, lines.size(), test);
    ...
    public static float longestLine(Font font, String input) {
        float longest = 0f;
        ...
        if (input != null && font != null) {
            ...
            for (int j = 0; j < data.length; j++) {
                ...
                if (data[j] == '\n') {
                    float pixelLength = font.getWidth(data, i, j - i);
            ...
            // Last control
            float pixelLength = font.getWidth(data, i, data.length - i);
