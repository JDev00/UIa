...
public class TextView extends View {
                ...
                .addEvent((v, s) -> {
                    switch (s) {
                        ...
                        case Event.KEY_PRESSED:
                            ...
                            if (key.isCode(Key.MASK_CTRL, Key.KEY_A)) {
                                hIndex = 0;
                                index = buffer.size();
                            ...
                            } else if (key.isCode(Key.MASK_SHIFT, Key.KEY_LEFT)) {
                                index = max(0, index - 1);
                            } else if (key.isCode(Key.MASK_SHIFT, Key.KEY_RIGHT)) {
                                index = min(index + 1, buffer.size());
                            } else if (key.isCode(Key.MASK_SHIFT, Key.KEY_UP)) {
                                int ind = getIndex(-1);
                                if (ind >= 0) index = ind;
                            } else if (key.isCode(Key.MASK_SHIFT, Key.KEY_DOWN)) {
                                int ind = getIndex(1);
                                if (ind >= 0) index = ind;
                            }
                            break;
    ...
    /**
     * Scroll text along x-axis.
     * <br>
     * This functionality is available only when text goes out of View's bounds.
     *
     * @param left true to scroll text on the left of one scrollerFactor unit
     * @see #getScrollerX()
     */

    public void scrollX(boolean left) {
        if (left) {
            scrollerX.setValue(scrollerX.getValue() - scrollerX.getFactor());
        } else {
            scrollerX.setValue(scrollerX.getValue() + scrollerX.getFactor());
        }
    }
    ...
    /**
     * Scroll text along y-axis
     * <br>
     * This functionality is available only when text goes out of View's bounds.
     *
     * @param up true to scroll text up of one scrollerFactor unit
     * @see #getScrollerY()
     */

    public void scrollY(boolean up) {
        if (up) {
            scrollerY.setValue(scrollerY.getValue() - scrollerY.getFactor());
        } else {
            scrollerY.setValue(scrollerY.getValue() + scrollerY.getFactor());
        }
    }
                ...
                if (i == length || chars[i] == '\n') {
                    ...
                    if (y + (lines + 2) * lineHeight >= top && y + (lines - 1) * lineHeight <= bot) {
                        graphic.drawText(chars, sol, eol - sol,
                                ...
                                y + (lines + 0.75f) * lineHeight);//0.75f
                    }
    ...
    protected int getIndex(int i) {// Bug
