...
import static uia.core.event.Event.*;
...
public class View {
    ...
    /**
     * Set the view's shape and set {@link #buildShape(Consumer, boolean)} to null
     *
     ...
     * Build a Shape
     *
     * @param build       a {@link Consumer}; it could be null
     * @param dynamically true to rebuild the Shape every time the view's draw is called
     */

    public void buildShape(Consumer<Shape> build, boolean dynamically) {
        if (dynamically) {
            dynShape = build;
        } else {
            dynShape = null;
            if (build != null) build.accept(shape);
        }
    }
    ...
    public void requestFocus() {
        if (!focus && isVisible()) {
            focus = true;
            updateEvent(Event.class, Event.FOCUS_GAINED);
        }
    }
    ...
    public void removeFocus() {
        if (focus) {
            focus = false;
            updateEvent(Event.class, FOCUS_LOST);
        }
    }
        ...
        if (compute
            ...
            if (over) {
                ...
                if (compute || (p.isConsumed() && p.getAction() == Pointer.EXITED)) {
                    updateEvent(Event.class, POINTER_LEAVE);
                }
                pointerEvent.clear();
            }
        }
    ...
    protected void updateKeys(boolean update) {
        ...
        if (update && isFocused() && !k.isConsumed()) {
            ...
            updateEvent(Event.class, k.getAction());
        }
    }
