...
public class StdAnimator implements Animator {
    ...
    protected boolean callEvent(Class<?> type, int state) {
        return EventQueue.accessEvent(eventQueue, type, e -> e.apply(StdAnimator.this, state));
    }
    ...
    public void update() {
        if (!pause && curr < animatorPath.size()) {
            ...
            if (movable.nearTo(next, Math.abs(ix) + Math.abs(iy))) {
                ...
                callEvent(NodeEvent.class, NodeEvent.NEXT_NODE);
                ...
                if (curr < animatorPath.size()) {// calculate the step for the next node
                    ...
                    callEvent(NodeEvent.class, NodeEvent.LAST_NODE);
                }
            } else {
                movable.add(ix, iy);
            }
