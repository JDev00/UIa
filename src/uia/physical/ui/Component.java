...
public final class Component implements View {
    ...
    public void update(View parent) {
        if (visible) {
            ...
            // geometry variables
            float[] p_bounds_s = parent.boundsShape();
            float s_w = expanse[0] * container[2] * p_bounds_s[2];
            float s_h = expanse[1] * container[3] * p_bounds_s[3];
            float d_x = (container[0] - 0.5f) * p_bounds_s[2];
            float d_y = (container[1] - 0.5f) * p_bounds_s[3];
            float d_rot = p_bounds[4];

            // bounds variables
            float b_rot = container[4] + p_bounds[4];
            float b_w = boundX(s_w, s_h, cos(b_rot), sin(b_rot));
            float b_h = boundY(s_w, s_h, cos(b_rot), sin(b_rot));
    ...
    private final float[] boundsShapeCopy = new float[5];
    ...
    public float[] boundsShape() {
        boundsShapeCopy[0] = geom.x - geom.scaleX;
        boundsShapeCopy[1] = geom.y - geom.scaleY;
        boundsShapeCopy[2] = 2 * geom.scaleX;
        boundsShapeCopy[3] = 2 * geom.scaleY;
        boundsShapeCopy[4] = container[4];
        return boundsShapeCopy;
    }
